            ...
            if(ctgnums){
                ...
                $.each(ctgnumArray,function (i,ctg1Id) {
                    ...
                    $.each(ctg2list,function (i,ctg2) {
                        var cata2link=$("<a href='#' style= 'color: #111; font-size: 12px;' class='aaa'>"+ctg2.name+"  ></a>");


                        console.log(cata2link.html());
                        var li=$("<li></li>");
                        var  ctg3List=ctg2["childes"];
                        var len=0;
                        $.each(ctg3List,function (i,ctg3) {
                            var cata3link = $("<a href=\"http://search.gmall.com/list.html?catalog3Id="+ctg3.catId+"\" style=\"color: #999;\">" + ctg3.name + "</a>");
                            li.append(cata3link);
